    ...
    <div className="pb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {categories.map((cat) => (
            <Link
              key={cat}
              href="/category"
              className="group bg-surface rounded-2xl p-3 flex flex-col justify-between shadow-sm hover:shadow-card hover:-translate-y-1 transition"
            >
              <div className="h-20 rounded-xl bg-gradient-to-br from-primary/10 via-bg to-secondary/10 mb-3" />
      ...
      <section id="collections" className="app-container mt-16">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link href="/category" className="text-xs font-semibold text-primary hover:underline">
            View all
          </Link>
        </div>
      ...
      <section className="app-container mt-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            { title: "Cozy Minimal Living Room", img: ROOM_PLACEHOLDER },
            { title: "Color Pop Bedroom", img: ROOM_PLACEHOLDER },
            { title: "Work From Home Setup", img: ROOM_PLACEHOLDER },
          ].map((room) => (
            <div
              key={room.title}
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white">
                <div>
                  <div className="font-heading font-semibold">{room.title}</div>
                  <div className="text-xs text-white/80">Shop the full look.</div>
                </div>
        ...
        <div className="rounded-3xl bg-gradient-to-r from-primary via-secondary to-accentTeal text-white px-6 md:px-10 py-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-card">
          <div>
            ...
            <p className="text-sm text-white/80">
              Be the first to know about new drops, décor tips & exclusive offers.
            </p>
          </div>
